...
public class SqlTimeConverter extends AbstractConverter {
	 ...
	 */
	public SqlTimeConverter(Object defaultValueAsNullInput) {
	 ...
	 */
	public SqlTimeConverter(Object defaultValueAsNullInput, Object defaultValueAsFailure) {
		super(defaultValueAsNullInput, defaultValueAsFailure);
	}

	/**
	 * 将value转换为java.sql.Time
	 * @return 转换后的数据( 类型：java.sql.Time )
	 */
	...
	public Object convert(Class type, Object value) {
		...
		try {
			if( value instanceof Date ) {
				...
				return new Time( TimeUtil.toDate( value.toString() ).getTime() );
			}
		} catch( Throwable t ) {
			if( _useDefaultAsFailure ) {
				return _defaultValueAsFailure;
			} else {
				throw new ConversionException( "Can not convert to java.sql.Time:" + value, t );
			}
		}
	}
