package com.zfsoft.hrm.overall.base.entity;

import java.util.Date;

public class OverAll {
	private String gh;
	private String xm;
	private String xmpy;
	private String dwh;
	private String xbm;
	private Date csrq;
	private String jg;
	private String mzm;
	private String sfzjlxm;
	private String bzlbm; //编制类别码
	public String getBzlbm() {
		return bzlbm;
	}
	public void setBzlbm(String bzlbm) {
		this.bzlbm = bzlbm;
	}
	public String getGh() {
		return gh;
	}
	public void setGh(String gh) {
		this.gh = gh;
	}
	public String getXm() {
		return xm;
	}
	public void setXm(String xm) {
		this.xm = xm;
	}
	public String getXmpy() {
		return xmpy;
	}
	public void setXmpy(String xmpy) {
		this.xmpy = xmpy;
	}
	public String getDwh() {
		return dwh;
	}
	public void setDwh(String dwh) {
		this.dwh = dwh;
	}
	public String getXbm() {
		return xbm;
	}
	public void setXbm(String xbm) {
		this.xbm = xbm;
	}
	public Date getCsrq() {
		return csrq;
	}
	public void setCsrq(Date csrq) {
		this.csrq = csrq;
	}
	public String getJg() {
		return jg;
	}
	public void setJg(String jg) {
		this.jg = jg;
	}
	public String getMzm() {
		return mzm;
	}
	public void setMzm(String mzm) {
		this.mzm = mzm;
	}
	public String getSfzjlxm() {
		return sfzjlxm;
	}
	public void setSfzjlxm(String sfzjlxm) {
		this.sfzjlxm = sfzjlxm;
	}
	public String getSfzjh() {
		return sfzjh;
	}
	public void setSfzjh(String sfzjh) {
		this.sfzjh = sfzjh;
	}
	public String getHyzkm() {
		return hyzkm;
	}
	public void setHyzkm(String hyzkm) {
		this.hyzkm = hyzkm;
	}
	public String getZzmmm() {
		return zzmmm;
	}
	public void setZzmmm(String zzmmm) {
		this.zzmmm = zzmmm;
	}
	public String getJkzkm() {
		return jkzkm;
	}
	public void setJkzkm(String jkzkm) {
		this.jkzkm = jkzkm;
	}
	public String getGwlbm() {
		return gwlbm;
	}
	public void setGwlbm(String gwlbm) {
		this.gwlbm = gwlbm;
	}
	public String getGwdjm() {
		return gwdjm;
	}
	public void setGwdjm(String gwdjm) {
		this.gwdjm = gwdjm;
	}
	public String getRzgwm() {
		return rzgwm;
	}
	public void setRzgwm(String rzgwm) {
		this.rzgwm = rzgwm;
	}
	public String getZjzw() {
		return zjzw;
	}
	public void setZjzw(String zjzw) {
		this.zjzw = zjzw;
	}
	public String getXzzw() {
		return xzzw;
	}
	public void setXzzw(String xzzw) {
		this.xzzw = xzzw;
	}
	public Date getCjgzsj() {
		return cjgzsj;
	}
	public void setCjgzsj(Date cjgzsj) {
		this.cjgzsj = cjgzsj;
	}
	public String getZwjb() {
		return zwjb;
	}
	public void setZwjb(String zwjb) {
		this.zwjb = zwjb;
	}
	private String sfzjh;
	private String hyzkm;
	private String zzmmm;
	private String jkzkm;
	private String gwlbm;
	private String gwdjm;
	private String rzgwm;
	private String zjzw;
	private String xzzw;
	private Date cjgzsj;
	private String zwjb;


}
